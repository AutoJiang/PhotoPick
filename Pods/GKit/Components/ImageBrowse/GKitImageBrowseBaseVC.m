...
@property (nonatomic,strong)GKitImageBrowseActionSheet *browseActionSheet;
@property (nonatomic,strong)GKitImageBrowseRemindView *browseRemindView;

@end

@implementation GKitImageBrowseBaseVC
...
- (void)showBrowseViewController
{
    ...
    {
        _snapshotView = [viewController.view snapshotViewAfterScreenUpdates:NO];
    }
...
- (void)initData
{
    for (GKitImageBrowseModel *browseItem in _browseItemArray)
    {
        ...
        if(_isEqualRatio)
        {
            ...
            {
                verticalRect = [browseItem.smallImageView.image gkit_imagebrowse_getBigImageRectSizeWithScreenWidth:MSS_SCREEN_WIDTH screenHeight:MSS_SCREEN_HEIGHT];
                horizontalRect = [browseItem.smallImageView.image gkit_imagebrowse_getBigImageRectSizeWithScreenWidth:MSS_SCREEN_HEIGHT screenHeight:MSS_SCREEN_WIDTH];
            }
        }
...
- (void)createBrowseView
{
    ...
    [_collectionView registerClass:[GKitImageBrowseCollectionViewCell class] forCellWithReuseIdentifier:@"MSSBrowserCell"];
    _collectionView.contentOffset = CGPointMake(_currentIndex * (_screenWidth + kBrowseSpace), 0);
    [_bgView addSubview:_collectionView];
    
    _pageCtrl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, _screenHeight - 30, _screenWidth, 10)];
    [_pageCtrl setUserInteractionEnabled:NO];
    [_pageCtrl setNumberOfPages:_browseItemArray.count];
    [_pageCtrl setCurrentPage:_currentIndex];
    [_pageCtrl setPageIndicatorTintColor:[UIColor grayColor]];
    [_pageCtrl setCurrentPageIndicatorTintColor:[UIColor whiteColor]];
    [_bgView addSubview:_pageCtrl];
    _pageCtrl.hidden = _browseItemArray.count == 1;
    
    _browseRemindView = [[GKitImageBrowseRemindView alloc]initWithFrame:_bgView.bounds];
    [_bgView addSubview:_browseRemindView];
}

- (CGRect)smallRect:(CGRect)bigRect baseRect:(CGRect)rect
{
    CGFloat scaleX = bigRect.size.width / rect.size.width;
    CGFloat scaleY = bigRect.size.height / rect.size.height;
    CGFloat minScale = MIN(scaleX, scaleY);
    CGFloat x = rect.origin.x+rect.size.width/2-bigRect.size.width/minScale/2;
    CGFloat y = rect.origin.y+rect.size.height/2-bigRect.size.height/minScale/2;
    CGFloat width = bigRect.size.width/minScale;
    CGFloat height = bigRect.size.height/minScale;
    if (!([self floatOk:x] && [self floatOk:y] && [self floatOk:width] && [self floatOk:height])) {
        return CGRectZero;
    }
...
- (BOOL)floatOk:(CGFloat)data
{
    return data >= -MAXFLOAT && data <= MAXFLOAT;
}
...
{
    GKitImageBrowseCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"MSSBrowserCell" forIndexPath:indexPath];
    if(cell)
    {        
        GKitImageBrowseModel *browseItem = [_browseItemArray objectAtIndex:indexPath.row];
        // 还原初始缩放比例
...
//        cell.zoomScrollView.zoomImageView.clipsToBounds = browseItem.smallImageView.clipsToBounds;
        [cell.loadingView gkit_imagebrowse_setFrameInSuperViewCenterWithSize:CGSizeMake(30, 30)];
        ...
        [self loadBrowseImageWithDataSource:_browseItemArray currentIndex:indexPath.row Cell:cell deviceOrientation:_currentOrientation];
        
        __weak __typeof(self)weakSelf = self;
        [cell tapClick:^(GKitImageBrowseCollectionViewCell *browseCell) {
            __strong __typeof(weakSelf)strongSelf = weakSelf;
            [strongSelf tap:browseCell];
        }];
        [cell longPress:^(GKitImageBrowseCollectionViewCell *browseCell) {
            __strong __typeof(weakSelf)strongSelf = weakSelf;
...
- (void)tap:(GKitImageBrowseCollectionViewCell *)browseCell
{
    // 移除通知
    ...
    for (GKitImageBrowseCollectionViewCell *cell in cellArray)
    {
    ...
    GKitImageBrowseModel *browseItem = _browseItemArray[indexPath.row];
    /*
    ...
    if(browseItem.smallImageView)
    {
        ...
        CGRect bigRect = browseCell.zoomScrollView.zoomImageView.frame;
        rect = [self smallRect:bigRect baseRect:rect];
...
- (void)longPress:(GKitImageBrowseCollectionViewCell *)browseCell
{
    [_browseActionSheet removeFromSuperview];
    _browseActionSheet = nil;
    __weak __typeof(self)weakSelf = self;
    _browseActionSheet = [[GKitImageBrowseActionSheet alloc]initWithTitleArray:@[@"保存图片到本地"] cancelButtonTitle:@"取消" didSelectedBlock:^(NSInteger index) {
        __strong __typeof(weakSelf)strongSelf = weakSelf;
...
- (void)deviceOrientationDidChange:(NSNotification *)notification
{
    ...
    if(orientation == UIDeviceOrientationPortrait || orientation == UIDeviceOrientationLandscapeLeft || orientation == UIDeviceOrientationLandscapeRight)
    {
        ...
        _pageCtrl.frame = CGRectMake(0, _screenHeight - 30, _screenWidth, 10);
...
- (void)browseActionSheetDidSelectedAtIndex:(NSInteger)index currentCell:(GKitImageBrowseCollectionViewCell *)currentCell
{    // 保存图片
    if(index == 0)
    {
    ...
    {
        GKitImageBrowseModel *currentBwowseItem = _browseItemArray[_currentIndex];
